...
| R2 | Which shell runs `--bind` commands | §0.9.3, Q17: "fzf runs the inner command with `sh -c`" | go-libs S1: fzf reads `$SHELL` and only falls back to `sh`. True since fzf 0.11.3. Fix by passing `--with-shell sh` (fzf ≥ 0.51.0). | go-libs wins. SPEC's claim was false; the fix makes it true. | **yes** — §0.5, §0.9.3, Q17 |
